...
public enum CameraManipulationModes
{
    Free,
    SelectAndTransform
}

public enum EnumNewWaypointMode
{
    SceneCamera,
    LastWaypoint,
    WaypointIndex,
...
public class CameraPathInspectorEditor : Editor
{
    ...
    private float time;
    private CameraManipulationModes cameraTranslateMode;
    private CameraManipulationModes cameraRotationMode;
    private CameraManipulationModes handlePositionMode;
    private EnumNewWaypointMode waypointMode;
    private int waypointIndex = 1;
    private EnumCurveType allCurveType = EnumCurveType.Custom;
    ...
    {
        cameraTranslateMode = (CameraManipulationModes)PlayerPrefs.GetInt("CPC_cameraTranslateMode", 1);
        cameraRotationMode = (CameraManipulationModes)PlayerPrefs.GetInt("CPC_cameraRotationMode", 1);
        handlePositionMode = (CameraManipulationModes)PlayerPrefs.GetInt("CPC_handlePositionMode", 0);
        waypointMode = (EnumNewWaypointMode) PlayerPrefs.GetInt("CPC_waypointMode", 0);
    ...
    void SetupReorderableList()
    {
        ...
        pointReorderableList.drawElementCallback = (rect, index, active, focused) =>
        {
            ...
            EnumCurveType tempP = (EnumCurveType)EditorGUI.EnumPopup(rect, t.points[index].curveTypePosition);
            ...
            //rect.x += rect.width + 2;
            EditorGUI.BeginChangeCheck();
            GUI.enabled = t.points[index].curveTypePosition == EnumCurveType.Custom;
            ...
            rect.width = (fullWidth - 22) / 3 - 1;
            EditorGUI.BeginChangeCheck();
            EnumCurveType temp = (EnumCurveType)EditorGUI.EnumPopup(rect, t.points[index].curveTypeRotation);
            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObject(t, "Changed enum value");
            ...
            //rect.x += rect.width + 2;
            EditorGUI.BeginChangeCheck();
            GUI.enabled = t.points[index].curveTypeRotation == EnumCurveType.Custom;
    ...
    void DrawBasicSettings()
    {
        ...
        afterLoopProperty.enumValueIndex = Convert.ToInt32(EditorGUILayout.EnumPopup((LoopCompletionAction)afterLoopProperty.intValue));
        GUI.enabled = true;
        GUILayout.EndHorizontal();
    ...
    void DrawVisualDropdown()
    {
        ...
        if (visualFoldout)
        {
            ...
            if (GUILayout.Button("Default colors"))
            {
                ...
                t.visual = new CameraPathVisuals();
            }
            GUILayout.EndVertical();
        }
        if (EditorGUI.EndChangeCheck())
    ...
    void DrawManipulationDropdown()
    {
        ...
        {
            GUILayout.BeginVertical("Box");
            cameraTranslateMode = (CameraManipulationModes)EditorGUILayout.EnumPopup("Waypoint Translation", cameraTranslateMode);
            cameraRotationMode = (CameraManipulationModes)EditorGUILayout.EnumPopup("Waypoint Rotation", cameraRotationMode);
            handlePositionMode = (CameraManipulationModes)EditorGUILayout.EnumPopup("Handle Translation", handlePositionMode);
            GUILayout.EndVertical();
        }
        if (EditorGUI.EndChangeCheck())
        {
    ...
    void DrawWaypointList()
    {
        ...
        GUILayout.BeginVertical("Box");
        GUILayout.BeginHorizontal();
        allCurveType = (EnumCurveType)EditorGUILayout.EnumPopup(allCurveType, GUILayout.Width(Screen.width / 3f));
        ...
        GUILayout.EndHorizontal();
        GUILayout.BeginHorizontal();
        GUI.enabled = allCurveType == EnumCurveType.Custom;
        ...
        EditorGUI.BeginChangeCheck();
        waypointMode = (EnumNewWaypointMode) EditorGUILayout.EnumPopup(waypointMode, waypointMode==EnumNewWaypointMode.WaypointIndex ? GUILayout.Width(Screen.width/4) : GUILayout.Width(Screen.width/2));
        if (waypointMode == EnumNewWaypointMode.WaypointIndex)
        {
    ...
    void DrawNextHandle(int i)
    {
        if (i < t.points.Count - 1 || loopedProperty.boolValue)
        {
            ...
            if (handlePositionMode == CameraManipulationModes.Free)
            {
#if UNITY_5_5_OR_NEWER
                var fmh_570_97_638642346229472936 = Quaternion.identity; posNext = Handles.FreeMoveHandle(t.points[i].position + t.points[i].handlenext, size, Vector3.zero, Handles.SphereHandleCap);
#else
    ...
    void DrawPrevHandle(int i)
    {
        if (i > 0 || loopedProperty.boolValue)
        {
            ...
            {
#if UNITY_5_5_OR_NEWER
                var fmh_622_97_638642346229502951 = Quaternion.identity; posPrev = Handles.FreeMoveHandle(t.points[i].position + t.points[i].handleprev, 0.1f * HandleUtility.GetHandleSize(t.points[i].position + t.points[i].handleprev), Vector3.zero, Handles.SphereHandleCap);
#else
    ...
    void DrawWaypointHandles(int i)
    {
        if (Tools.current == Tool.Move)
        {
            ...
            if (cameraTranslateMode == CameraManipulationModes.SelectAndTransform)
            {
            ...
            {
#if UNITY_5_5_OR_NEWER
                var fmh_678_68_638642346229511355 = (Tools.pivotRotation == PivotRotation.Local) ? t.points[i].rotation : Quaternion.identity; pos = Handles.FreeMoveHandle(t.points[i].position, HandleUtility.GetHandleSize(t.points[i].position) * 0.2f, Vector3.zero, Handles.RectangleHandleCap);
#else
        ...
        else if (Tools.current == Tool.Rotate)
        {
            ...
            if (cameraRotationMode == CameraManipulationModes.SelectAndTransform)
            {
    ...
    void DrawSelectionHandles(int i)
    {
        ...
        {
            if (cameraTranslateMode == CameraManipulationModes.SelectAndTransform && Tools.current == Tool.Move
                || cameraRotationMode == CameraManipulationModes.SelectAndTransform && Tools.current == Tool.Rotate)
            {
